// ─────────────────────────────────────────────────────────────────────────────
// MIRRORED COPY — do not edit directly.
//
// This file must stay byte-for-byte identical to
// supabase/functions/_shared/scoring.ts, which is the canonical source
// (also used by frontend/src/lib/rosterScoring.ts). It exists here, nested
// under generate-roster/, only because this function is deployed through
// the Supabase Dashboard's function editor, which bundles a single
// function's own file tree and does not resolve imports that reach outside
// it (e.g. the sibling '../_shared/scoring.ts'). The Supabase CLI does not
// have this limitation and would work fine against the canonical file
// directly — if this function is ever switched to CLI-based deploys, this
// mirrored copy and generate-roster/index.ts + solver.ts's './_shared/...'
// imports can be removed in favor of '../_shared/scoring.ts'.
//
// Any change to the canonical file's scoring logic must be copied here too.
// ─────────────────────────────────────────────────────────────────────────────
